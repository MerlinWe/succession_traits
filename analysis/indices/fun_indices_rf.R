...
lower_results_50 <- train_and_predict(stratified_data_climate_50$lower_data_50, indices, covariates, hyper_grid)
upper_results_50 <- train_and_predict(stratified_data_climate_50$upper_data_50, indices, covariates, hyper_grid)
...
plot_partial_dependence_climate_50 <- function(lower_results_50, upper_results_50, index) {
	
	# Extract models
	lower_model_50 <- lower_results_50$best_models[[which(indices == index)]]
	upper_model_50 <- upper_results_50$best_models[[which(indices == index)]]
	...
	# Extract R-squared values
	lower_r2_50 <- lower_results_50$r_squared_values[[which(indices == index)]]
	upper_r2_50 <- upper_results_50$r_squared_values[[which(indices == index)]]
	...
	plot_50 <- ggplot(pdp_data_50, aes(x = standage, y = yhat, color = group, shape = group)) +
		...
		facet_wrap(~group, scales = "fixed", ncol = 3, nrow = 1)
	
	r2_tibble_50 <- tibble(index = index,
...
# Generate plot data for all indices for 50% quantiles
pdp_plots_climate_50 <- list()
r2_values_50 <- tibble(index = character(), group = character(), r2 = numeric())
for (index in indices) {
	result_50 <- plot_partial_dependence_climate_50(lower_results_50, upper_results_50, index)
	pdp_plots_climate_50[[index]] <- result_50$plot
	r2_values_50 <- bind_rows(r2_values_50, result_50$r2_tibble)
}

# Combine the pdp plots using patchwork for 50% quantiles
pdp_plots_climate_50 <- wrap_plots(pdp_plots_climate_50, ncol = 3, nrow = 1) +
	plot_annotation(
		theme = theme(
			text = element_text(family = "Arial"),
			plot.title = element_text(face = "bold")
...
r2_plot_50 <- ggplot(r2_values_50, aes(x = index, y = r2, fill = group)) +
	geom_bar(stat = "identity", position = position_dodge(width = 0.5), colour = "black") +
...
lower_results_halves <- train_and_predict(stratified_data_climate_halves$lower_data_halves, indices, covariates, hyper_grid)
upper_results_halves <- train_and_predict(stratified_data_climate_halves$upper_data_halves, indices, covariates, hyper_grid)
...
plot_partial_dependence_climate_halves <- function(lower_results_halves, upper_results_halves, index) {
	
	# Extract models
	lower_model_halves <- lower_results_halves$best_models[[which(indices == index)]]
	upper_model_halves <- upper_results_halves$best_models[[which(indices == index)]]
	...
	# Extract R-squared values
	lower_r2_halves <- lower_results_halves$r_squared_values[[which(indices == index)]]
	upper_r2_halves <- upper_results_halves$r_squared_values[[which(indices == index)]]
	...
	r2_tibble_halves <- tibble(index = index,
...
# Generate plot data for all indices for halves
pdp_plots_climate_halves <- list()
r2_values_halves <- tibble(index = character(), group = character(), r2 = numeric())
for (index in indices) {
	result_halves <- plot_partial_dependence_climate_halves(lower_results_halves, upper_results_halves, index)
	pdp_plots_climate_halves[[index]] <- result_halves$plot
	r2_values_halves <- bind_rows(r2_values_halves, result_halves$r2_tibble)
}

# Combine the pdp plots using patchwork for halves
pdp_plots_climate_halves <- wrap_plots(pdp_plots_climate_halves, ncol = 3, nrow = 1) +
	plot_annotation(
		theme = theme(
			text = element_text(family = "Arial"),
			plot.title = element_text(face = "bold")
...
r2_plot_halves <- ggplot(r2_values_halves, aes(x = index, y = r2, fill = group)) +
	geom_bar(stat = "identity", position = position_dodge(width = 0.5), colour = "black") +
