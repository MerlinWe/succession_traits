...
fia_clean <- fia_clean %>%
	...
	select(-Name) %>%
	select(starts_with("fun_"), standage, ecoregion, biome, managed, 
	...
	as_tibble() %>%
	write_csv("fun_rf_clean.csv", file = "/Volumes/ritd-ag-project-rd01pr-dmayn10/merlin/data/fia_traits/sub/fun_rf_clean.csv")
