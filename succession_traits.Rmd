...
-   **Climate data** gathered for every plot location provided from the Crowther Lab \@ ETH Zurich. We mainly use the *CHELSA_BIO* data.
...
Finally, we summarise the entire dataset to the plot (PID) level, where eventually one row is one plot. We summarise the weighted mean (by basal area) and standard deviation. For every plot we then know the biome, inventory year, ownership, if it is managed, the location ID, the summary statistics of the functional traits wood density, lead nitrogen, bark thickness, seed dry mass, shade tolerance, specific leaf area, diameter, and basal area per acre. We also know the total of the carbon above the ground and the stand age. For each plot community we also know the functional dispersion, diversion, and evenness, and our custom resource use score.
...
grouping_vars <- c("STDAGE", "INVYR", "FORTYPCD", "biome","ownership", "managed", "ll_id")
...
fia <- fia %>%
	...
	group_by(accepted_bin, PID, STDAGE, INVYR, FORTYPCD, biome, ownership, managed, ll_id) %>%
...
fia <- dat %>%
	...
	group_by(PID, STDAGE, INVYR, FORTYPCD, biome, ownership, managed, ll_id) %>%
	...
	summarise(
	 ...
	 # Shade tolerance 
		...
		sd_shade_tolerance = sd(shade_tolerance, na.rm = TRUE),
...
```{r prep FIA data, echo=TRUE, warning = FALSE, message=FALSE, eval=FALSE}
...
Given the large size, the climate data is being added after the preparation pieline to speed up computation:
...
```{r plot score, echo = FALSE, warning=FALSE, message=FALSE, fig.width=12, fig.height=5, fig.cap = "Resource use strategy vs stand age"}
...
fia_clean %>%
	...
	select(PID, standage, sd_wood_density, sd_bark_thickness, sd_conduit_diam, sd_leaf_n, 
				 sd_shade_tolerance, sd_specific_leaf_area, sd_seed_dry_mass, sd_dia, sd_height) %>%
	# Pivot longer
	pivot_longer(
